...
export const messages: Record<SupportedLanguage, Record<MessageKey, string>> = {
  "zh-CN": {
    ...
    "connection.modal.field.oceanBaseProtocol.help.primary":
      "MySQL 租户选择 MySQL；Oracle 租户选择 Oracle。PinkHunkDB 会根据端口自动选择：OB MySQL wire 端口走 OBClient capability 注入（与 Navicat 相同路径），OBProxy Oracle listener 端口走标准 TNS。",
    "connection.modal.field.oceanBaseProtocol.help.connectionAttributes":
      "如果 Oracle 租户连接报「Error 1235」或 OBClient 握手失败，可在「连接参数」字段通过 {attributes} 覆盖 PinkHunkDB 默认注入的 OBClient capability。",
    ...
    "connection.modal.jvm.mode.agent.description":
      "通过 PinkHunkDB Java Agent 提供更完整的增强能力。",
    ...
    "connection.modal.jvm.agent.description":
      "连接 PinkHunkDB Java Agent 管理端口，用于增强采集和诊断链路。",
    ...
    "connection.modal.jvm.agent.address.help":
      "目标 Java 服务需要以 -javaagent 方式启动 PinkHunkDB Agent。",
    "connection.modal.jvm.agent.address.placeholder":
      "例如：http://127.0.0.1:19090/PinkHunkDB/agent/jvm",
    ...
    "connection.modal.jvm.diagnostic.bridgeAddress.help":
      "例如：http://127.0.0.1:19091/PinkHunkDB/diag",
    ...
    "driver.guidance.localImportSingleFileHelp":
      "行内“导入驱动包”仅用于单个驱动文件/总包（如 `mariadb-driver-agent`、`mariadb-driver-agent.exe`、`PinkHunkDB-DriverAgents.zip`），不支持直接导入 JDBC Jar；批量导入请使用上方“导入驱动目录”。",
    "driver.guidance.customConnectionDriverHelp":
      "已支持: mysql, starrocks, oceanbase, postgres, opengauss, sqlite, oracle, dm, kingbase；别名支持 postgresql/pgx、open_gauss/open-gauss、dm8、kingbase8/kingbasees/kingbasev8。请填写 PinkHunkDB 已注册的 Go database/sql 驱动名，不能直接填写系统 ODBC/JDBC 驱动名或导入 JDBC Jar。",
    ...
    "driver.modal.network.unreachable.description":
      "当前可能能访问 GitHub 页面，但驱动包下载会跳转到资产域名。请优先在 PinkHunkDB 顶部“代理”中启用全局代理（填写代理应用本地地址和端口）。",
  ...
  "en-US": {
    ...
    "connection.modal.field.oceanBaseProtocol.help.primary":
      "Choose MySQL for MySQL tenants and Oracle for Oracle tenants. PinkHunkDB auto-selects by port: OB MySQL wire ports use OBClient capability injection (same path as Navicat), and OBProxy Oracle listener ports use standard TNS.",
    "connection.modal.field.oceanBaseProtocol.help.connectionAttributes":
      "If an Oracle tenant reports \"Error 1235\" or the OBClient handshake fails, override PinkHunkDB's default OBClient capability injection through {attributes} in the Connection parameters field.",
    ...
    "connection.modal.jvm.mode.agent.description":
      "Use PinkHunkDB Java Agent for richer enhanced capabilities.",
    ...
    "connection.modal.jvm.agent.description":
      "Connect to the PinkHunkDB Java Agent management port for enhanced collection and diagnostics.",
    "connection.modal.jvm.agent.address.label": "Agent address",
    "connection.modal.jvm.agent.address.required":
      "Enter the Agent address when Agent mode is enabled",
    "connection.modal.jvm.agent.address.help":
      "The target Java service must start PinkHunkDB Agent with -javaagent.",
    "connection.modal.jvm.agent.address.placeholder":
      "For example: http://127.0.0.1:19090/PinkHunkDB/agent/jvm",
    ...
    "connection.modal.jvm.diagnostic.transport.agentBridge.description":
      "Bridge diagnostic commands through PinkHunkDB Agent.",
    "connection.modal.jvm.diagnostic.transport.arthasTunnel.description":
    ...
    "connection.modal.jvm.diagnostic.bridgeAddress.help":
      "For example: http://127.0.0.1:19091/PinkHunkDB/diag",
    "connection.modal.jvm.diagnostic.targetId.agentId.label":
    ...
    "connection.modal.network.keepAliveInterval.help":
      "PinkHunkDB pings established cached connections at this interval. Default is 240 minutes.",
    "connection.modal.network.keepAliveInterval.range":
    ...
    "driver.guidance.localImportSingleFileHelp":
      "The inline \"Import driver package\" action only accepts a single driver file or bundle (for example `mariadb-driver-agent`, `mariadb-driver-agent.exe`, `PinkHunkDB-DriverAgents.zip`). It does not import JDBC Jar directly. Use \"Import driver directory\" above for batch import.",
    "driver.guidance.customConnectionDriverHelp":
      "Supported: mysql, starrocks, oceanbase, postgres, opengauss, sqlite, oracle, dm, kingbase; aliases include postgresql/pgx, open_gauss/open-gauss, dm8, kingbase8/kingbasees/kingbasev8. Enter a Go database/sql driver name already registered by PinkHunkDB. Do not enter a system ODBC/JDBC driver name directly or import a JDBC Jar.",
    ...
    "driver.modal.network.unreachable.description":
      "GitHub pages may still open, but driver package downloads jump to asset hosts. Enable the global proxy from the PinkHunkDB top bar first by entering the local proxy address and port used by the proxy app.",
