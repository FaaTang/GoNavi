...
func ParseRemoteMCPClientConfigOptions(args []string) (RemoteMCPClientConfigOptions, error) {
	...
	fs.StringVar(&options.LocalAddr, "addr", options.LocalAddr, "local HTTP listen address for PinkHunkDB")
	fs.StringVar(&options.Path, "path", options.Path, "local and public MCP path")
	fs.StringVar(&options.GoNaviCommand, "gonavi-command", options.GoNaviCommand, "PinkHunkDB application command on Windows")
...
func normalizeRemoteMCPClientConfigOptions(options RemoteMCPClientConfigOptions) RemoteMCPClientConfigOptions {
	...
	if options.GoNaviCommand == "" {
		options.GoNaviCommand = "PinkHunkDB.exe"
	}
...
func RenderRemoteMCPClientConfig(options RemoteMCPClientConfigOptions) (string, error) {
	...
	lines := []string{
		fmt.Sprintf("PinkHunkDB MCP 远程接入配置 - %s", normalized.DisplayName),
		"",
		"云端 Agent 配置（不要写数据库账号密码）：",
		string(configJSON),
		"",
		"Windows 本机启动 PinkHunkDB MCP HTTP：",
		launch,
		"",
		"独立 MCP Server 启动方式：",
		standalone,
		...
		"- 如明确去掉 --schema-only 开放 execute_sql，它仍受 PinkHunkDB AI 安全控制约束，写操作必须显式传 allowMutating=true。",
	}
