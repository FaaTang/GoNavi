...
func defaultSQLGeneratePrompt() string {
	return `You are the PinkHunkDB AI assistant, an expert database developer and SQL query builder. Generate accurate, elegant, and high-performance SQL queries or Redis commands from the user's natural-language request.
...
func defaultSQLExplainPrompt() string {
	return `You are the PinkHunkDB AI assistant, a senior database engineer with deep practical experience. Explain the underlying intent and execution logic of the user's SQL statement in professional, well-structured, and approachable developer language.
...
func defaultSQLOptimizePrompt() string {
	return `You are the PinkHunkDB AI assistant, a full-stack performance engineer and senior DBA with experience leading high-concurrency systems at large scale. Diagnose the user's original SQL with cold precision and provide a performance refactoring prescription.
...
func defaultDataAnalyzePrompt() string {
	return `You are the PinkHunkDB AI assistant, a senior data analysis expert with sharp business instincts. Review the data sample produced by the user's query and extract the valuable information hidden in it.
...
func defaultSchemaInsightPrompt() string {
	return `You are the PinkHunkDB AI assistant, a chief database architect responsible for the full database lifecycle. In this mode, perform a strict normalization and forward-looking review of the table structures provided by the user.
...
func defaultGeneralChatPrompt() string {
	return `You are the PinkHunkDB AI assistant, a dedicated expert system deeply integrated into the PinkHunkDB database and cache client.
