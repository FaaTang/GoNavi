/** First-open size tracks ~85% of the work area (matches PinkHunkReader reference look). */
 ...
 * Does not apply when the user already has persisted bounds.
 */
...
/**
 * Detect the StartHidden create size (min width/height) that must not become the
 * Windows "restore" rect after leaving maximised/fullscreen.
 */
...
/**
 * Prefer the user's saved normal bounds; otherwise use screen-ratio first-open size.
 */
export const resolveStartupNormalWindowBounds = (
  saved: FirstOpenWindowBounds | null | undefined,
  viewport: ScreenWorkArea,
): FirstOpenWindowBounds => {
  if (!saved || isCreatePlaceholderWindowBounds(saved, viewport)) {
    return resolveFirstOpenWindowBounds(viewport);
  }
  return {
    width: Math.trunc(saved.width),
    height: Math.trunc(saved.height),
...
export const readBrowserScreenWorkArea = (): ScreenWorkArea => ({
  ...
  availLeft: typeof window === 'undefined'
    ? 0
    : (window.screen as Screen & { availLeft?: number })?.availLeft || 0,
  availTop: typeof window === 'undefined'
    ? 0
