...
describe('TableOverview v2 context menu', () => {
  it('renders card and list table context menus through a measured portal', () => {
    ...
    expect(cardSource).toContain('onContextMenu={(event) => openV2OverviewContextMenu(event, table)}');
    expect(listSource).toContain('onContextMenu={(event) => openV2OverviewContextMenu(event, table)}');
