...
const QueryEditor: React.FC<{ tab: TabData; isActive?: boolean }> = ({ tab, isActive = true }) => {
  ...
  const refreshObjectDecorations = useCallback((maxTextLength = QUERY_EDITOR_OBJECT_DECORATION_MAX_TEXT_LENGTH) => {
      ...
      for (const candidate of candidates) {
          ...
          const inlineClassName = hoverTarget.kind === 'column'
              ? 'PinkHunkDB-query-editor-column-token'
              : hoverTarget.kind === 'database'
                  ? 'PinkHunkDB-query-editor-db-token'
                  : 'PinkHunkDB-query-editor-object-token';
  ...
  const showObjectInfoAtPosition = useCallback((position?: { lineNumber: number; column: number } | null) => {
      ...
      editor.trigger?.('PinkHunkDB-hover', 'editor.action.showHover', null);
      return true;
  }, []);
  ...
  const registerShowObjectInfoAction = useCallback(() => {
      ...
      objectHoverActionRef.current = editor.addAction({
          id: 'PinkHunkDB.queryEditor.showObjectInfo',
  ...
  const insertTextIntoEditorAtPosition = useCallback((text: string, position?: { lineNumber: number; column: number } | null) => {
      ...
      editor.executeEdits?.('PinkHunkDB-sidebar-drop', [{
          range: new monaco.Range(
  ...
  const handleDuplicateSelectionOrLine = useCallback(() => {
      ...
      if (hasSelection) {
          ...
          editor.executeEdits?.('PinkHunkDB.duplicateSelectionOrLine', [{
              range: insertRange,
              text: selectedText,
              forceMoveMarkers: true,
          }]);
  ...
  const handleEditorDidMount: OnMount = (editor, monaco) => {
      ...
      const applyNavigationHoverStateAtPosition = (targetPosition: { lineNumber: number; column: number } | null) => {
          ...
          linkDecorationIdsRef.current = editor.deltaDecorations(
              ...
              decorations.map((item) => ({
                  ...
                  options: {
                      inlineClassName: 'PinkHunkDB-query-editor-link-hint',
                  },
              })),
          );
      ...
      const handleImeCompositionEnd = (rawEvent: Event) => {
          ...
          imeCompositionFallbackTimerRef.current = setTimeout(() => {
              ...
              editor.executeEdits?.('PinkHunkDB-ime-composition-fallback', [{
                  range: fallbackRange,
                  text: committedText,
                  forceMoveMarkers: true,
              }]);
      ...
      if (selectStatementBinding?.enabled && selectStatementBinding.combo) {
          ...
          if (keyBinding) {
              selectCurrentStatementActionRef.current = editor.addAction({
                  id: 'PinkHunkDB.selectCurrentStatement',
      ...
      if (saveBinding?.enabled && saveBinding.combo) {
          ...
          if (keyBinding) {
              saveQueryActionRef.current = editor.addAction({
                  id: 'PinkHunkDB.saveQuery',
                  label: buildQueryEditorMonacoActionLabel('app.shortcuts.action.saveQuery.label'),
                  keybindings: [keyBinding.keyMod | keyBinding.keyCode],
                  run: () => {
                      window.dispatchEvent(new CustomEvent('PinkHunkDB:save-active-query'));
                  },
              });
          }
      }
      ...
      if (toggleResultsBinding?.enabled && toggleResultsBinding.combo) {
          ...
          if (keyBinding) {
              toggleQueryResultsPanelActionRef.current = editor.addAction({
                  id: 'PinkHunkDB.toggleQueryResultsPanel',
      ...
      if (duplicateBinding?.enabled && duplicateBinding.combo) {
          ...
          if (keyBinding) {
              duplicateSelectionOrLineActionRef.current = editor.addAction({
                  id: 'PinkHunkDB.duplicateSelectionOrLine',
  ...
  const formatSettingsMenu: MenuProps['items'] = [
      ...
      { type: 'divider' },
      {
          ...
          onClick: () => window.dispatchEvent(new CustomEvent('PinkHunkDB:open-snippet-settings')),
      },
      {
          key: 'shortcut-settings',
          label: translate('query_editor.format.shortcut_settings'),
          onClick: () => window.dispatchEvent(new CustomEvent('PinkHunkDB:open-shortcut-settings')),
      },
  ];
  ...
  const requestRun = useCallback(() => {
      ...
      const dispose = mountSqlExecutionChooser(editor, monaco, {
          ...
          onOpenSettings: () => {
              closeSqlExecutionChooser();
              window.dispatchEvent(new CustomEvent('PinkHunkDB:open-query-execution-settings'));
          },
          translate,
      });
  ...
  useEffect(() => {
      ...
      window.addEventListener('PinkHunkDB:run-active-query', handleRunActiveQuery as EventListener);
      return () => {
          window.removeEventListener('PinkHunkDB:run-active-query', handleRunActiveQuery as EventListener);
      };
  }, [isActive, requestRun]);
  ...
  useEffect(() => {
      ...
      window.addEventListener('PinkHunkDB:insert-sql-to-tab', handleInsertSql as EventListener);
      return () => window.removeEventListener('PinkHunkDB:insert-sql-to-tab', handleInsertSql as EventListener);
  }, [tab.id, requestRun]);
  ...
  useEffect(() => {
      ...
      window.addEventListener('PinkHunkDB:save-active-query', handleSaveActiveQuery as EventListener);
      return () => {
          window.removeEventListener('PinkHunkDB:save-active-query', handleSaveActiveQuery as EventListener);
      };
  ...
  useEffect(() => {
      ...
      window.addEventListener('PinkHunkDB:show-sql-execution-log', handleOpenSqlExecutionLog as EventListener);
      return () => {
          window.removeEventListener('PinkHunkDB:show-sql-execution-log', handleOpenSqlExecutionLog as EventListener);
      };
  }, [handleShowSqlExecutionLog]);
  ...
  useEffect(() => {
      ...
      window.addEventListener(
          'PinkHunkDB:open-query-execution-settings',
