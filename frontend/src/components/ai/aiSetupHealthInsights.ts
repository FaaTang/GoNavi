...
export const buildAISetupHealthSnapshot = (params: {
  ...
  if (mcpSnapshot.currentClientCount === 0) {
    appendUnique(warnings, translateInspectionCopy(
      ...
      'Claude Code / Codex is not connected to the current PinkHunkDB MCP as a local client yet; OpenClaw/Hermans need a remote bridge',
    ));
    appendUnique(nextActions, translateInspectionCopy(
      translate,
      'ai_chat.inspection.setup.next_action.connect_external_client',
      'To let external Agents use PinkHunkDB MCP, connect local clients such as Claude Code/Codex or configure a remote MCP bridge for cloud Agents',
    ));
  }
