...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --drivers)
      ...
      shift 2
      ;;
    --skip-if-unchanged-since)
      skip_unchanged_since="${2:-}"
      shift 2
      ;;
    --decide-skip-only)
      decide_skip_only=1
...
generated_platform_stamp() {
  local file="$1"
  [[ -f "$file" ]] || return 1
  sed -n 's/^\/\/ Generated for platform: //p' "$file" | head -n 1
}
...
if maybe_skip_unchanged_generation; then
  exit 0
fi
...
// Generated for platform: ${target_platform}
